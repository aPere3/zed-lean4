...
//! tags. `RpcPtr` handles are dropped (no subexpression inspection yet),
//! except lazy trace children, whose pointers the proxy uses for bounded
//! eager expansion.
...
pub fn conv_msg(tt: &Value, diff: Option<&str>, seed: u64, depth: u32, out: &mut Vec<MsgSeg>) {
    if let Some(s) = tt["text"].as_str() {
        ...
        for (i, p) in parts.iter().enumerate() {
            conv_msg(p, diff, mix(seed, i as u64), depth, out);
        }
    } else if let Some(pair) = tt["tag"].as_array()
        && pair.len() == 2
    {
        ...
        if !embed["expr"].is_null() {
            ...
            out.push(MsgSeg::Trace(conv_trace(&embed["trace"], seed, depth)));
            ...
            conv_msg(src, diff, mix(seed, 1), depth, out);
        } else {
            conv_msg(alt, diff, mix(seed, 1), depth, out);
        }
    }
}

fn conv_trace(tr: &Value, seed: u64, depth: u32) -> TraceNode {
    let cls = tr["cls"].as_str().unwrap_or("trace").to_string();
    let id = mix(seed, fnv1a(cls.as_bytes())) as u32;
    let mut header = Vec::new();
    conv_msg(&tr["msg"], None, mix(id as u64, 1), depth, &mut header);
    ...
    if let Some(strict) = tr["children"]["strict"].as_array() {
        node.children = conv_children(strict, id as u64, depth);
...
pub fn conv_children(list: &[Value], seed: u64, depth: u32) -> Vec<Vec<MsgSeg>> {
    list.iter()
        .enumerate()
        .map(|(i, c)| {
            let mut out = Vec::new();
            conv_msg(c, None, mix(seed, i as u64 + 2), depth + 1, &mut out);
            out
        })
        .collect()
}
...
pub fn conv_interactive_diag(v: &Value, index: usize) -> Diag {
    ...
    conv_msg(&v["message"], None, seed, 0, &mut message);
    Diag {
        line: line as u32,
